...
findings_task = Task(
    description=(
        ...
        "SCHRITT A: searchsploit '<service> <version>' — nie nur den Service-Namen allein.\n"
        "SCHRITT B: Wenn searchsploit leer oder kein passender Treffer → nvd_cve_search mit "
        "demselben '<service> <version>' aufrufen (z.B. 'Apache 2.4.51', 'OpenSSH 8.2p1').\n"
        "SCHRITT C: Wenn NVD Treffer liefert → DuckDuckGo '<CVE-ID> exploit PoC' zur Bestätigung.\n\n"
        "RELEVANZ-PRÜFUNG: Prüfe bei jedem Treffer ob Produktname UND Versionsnummer passen. "
